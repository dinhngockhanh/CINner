...
%--------------------------------------Compare chromosome strand alleles
    ...
    for chrom=1:N_chromosomes
        chrom_ploidy    = ploidy_chrom_1(chrom);
        for strand=1:chrom_ploidy
            if (size(ploidy_allele_1{chrom,strand},1)~=size(ploidy_allele_2{chrom,strand},1))||(size(ploidy_allele_1{chrom,strand},2)~=size(ploidy_allele_2{chrom,strand},2))
                output  = 0;
                return;
            end
...
%----------------------------------------------------Compare driver maps
    ...
    if (size(driver_map_1,1)~=size(driver_map_2,1))||(size(driver_map_1,2)~=size(driver_map_2,2))
        output          = 0;
        return;
    end
