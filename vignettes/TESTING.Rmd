...
prob_CN_whole_genome_duplication    <- 5e-6
...
# vec_driver_s[2]   <- 0.007
# vec_driver_s[3]   <- 0.007

vec_driver_s      <- rep(0.01,length(vec_driver_genes))
vec_driver_s[1]   <- 0.02
vec_driver_s[2]   <- 0.015
vec_driver_s[3]   <- 0.015
...
stage_final       <- 3
simulation        <- SIMULATOR_FULL_PROGRAM_one_simulation(MODEL_NAME,stage_final)



PLOT_total_population_vs_input(simulation,MODEL_NAME,vec_time_plot=seq(0,84,by=1),unit='year')


PLOT_clonal_evolution(simulation,vec_time_plot=seq(0,84,by=1),unit='year')
