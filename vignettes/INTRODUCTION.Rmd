...
The library can be installed and loaded by running:
...
The simulator's efficiency is based partly on its use of the tau-leaping algorithm, which simulates the evolution in fixed steps instead of the exact Gillespie algorithm. As such, it requires a suggested time step. If not specified, the time step is half of the cell lifespan. For statistical accuracy, it should not be higher. However, setting significantly low numbers for time step will lead to increased simulation run time.
...
- `save_simulation`: All output variables from each simulation are saved in separate `rda` files if set to `TRUE`.
