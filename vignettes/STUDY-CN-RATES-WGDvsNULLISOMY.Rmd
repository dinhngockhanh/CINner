...
T_0 <- list(0, "year")
T_end <- list(1, "year")
Table_sample <- data.frame(Sample_ID = c("SA01"), Cell_count = c(100), Age_sample = c(1))
# Table_sample <- data.frame(Sample_ID = c("SA01"), Cell_count = c(Inf), Age_sample = c(1))
...
arm_gains <- sample(length(arm_s), round(length(arm_s) / 2), replace = FALSE)
arm_losses <- setdiff(1:length(arm_s), arm_gains)
arm_s[arm_gains] <- 1 + 0.3
arm_s[arm_losses] <- 1 - 0.3
...
model_name <- "CN-STUDY-WGDvsNULLISOMY"
