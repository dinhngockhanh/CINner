#' @export
fitting_PCAWG_ALL_AT_ONCE <- function(model_name,
                                      model_variables,
                                      copynumber_PCAWG,
                                      n_cores = NULL,
                                      n_samples = NULL) {
    ...
    model_variables_fitting$selection_model$Value[which(model_variables_fitting$selection_model$Variable == "selection_model")] <- "chrom-arm-and-driver-gene-selection-diploid-base"
    ...
    PCAWG_delta_genome <- gainloss_PCAWG(
        copynumber_PCAWG,
    ...
    for (i in 1:10) {
        ...
        PCAWG_delta <- list()
        PCAWG_delta$delta_gain <- PCAWG_delta_genome$delta_gain[vec_loc]
        PCAWG_delta$delta_loss <- PCAWG_delta_genome$delta_loss[vec_loc]
        ...
        for (col in 1:ncol(sim_param)) {
            if (col <= length(list_arms)) {
                #   Simulate parameters for chromosome arms
                sim_param[, col] <- runif(fit_ABC_count, min = range_arm_s[1], max = range_arm_s[2])
            } else {
                #   Simulate parameters for driver genes
                gene_role <- model_variables_chrom$driver_library$Gene_role[col - length(list_arms)]
        ...
        if (fit_ABC_routine == "abc-rejection") {
            ...
            ABC$chromosome_arm_library <- model_variables_chrom$chromosome_arm_library
            ABC$driver_library <- model_variables_chrom$driver_library
        ...
        #---Fit by ABC-random forest
            ...
            cat("=================================================================================\n")
            cat("=================================================================================\n")
            cat(paste("Fitting chromosome ", chromosome_target, " with ", numCores, " cores, ABC-random forests with ", fit_ABC_count, " simulations\n", sep = ""))
            model_variables_chrom <<- model_variables_chrom
            PCAWG_N_cases <<- PCAWG_N_cases
            sim_param <<- sim_param
            func_ABC <<- func_ABC
            ...
            vec_para_alias <- paste("para", 1:(length(list_arms) + length(list_genes)), sep = "")
            vec_bin_alias <- paste("gainloss_", 1:length(target_PCAWG), sep = "")

            obs_rf <- data.frame(matrix(target_PCAWG, nrow = 1))
            colnames(obs_rf) <- vec_bin_alias
            #   Perform ABC-random forests
            ABC_RF <- list()
            for (para in 1:ncol(sim_param)) {
                #   For each parameter...
                ...
                model_rf <- regAbcrf(formula = f, data_rf, paral = TRUE, ncores = (max(detectCores() - 1, 1)))
                #   Get posterior statistics for this parameter
                post_rf <- predict(model_rf, obs_rf, data_rf, paral = TRUE, ncores = (max(detectCores() - 1, 1)))
            ...
            filename <- paste(model_name, "_fitting_chr", chromosome_target, ".rda", sep = "")
            ABC_RF$target_PCAWG <- target_PCAWG
            ABC_RF$PCAWG_delta <- PCAWG_delta
            ABC_RF$sim_param <- sim_param
            ABC_RF$sim_stat <- sim_stat
            ABC_RF$list_arms <- list_arms
            ABC_RF$list_genes <- list_genes
            ABC_RF$model_variables_chrom <- model_variables_chrom
            # ABC_RF$PCAWG_chromosome <- PCAWG_chromosome
            ABC_RF$PCAWG_N_cases <- PCAWG_N_cases
            ABC_RF$chromosome_arm_library <- model_variables_chrom$chromosome_arm_library
            ABC_RF$driver_library <- model_variables_chrom$driver_library
...
#' @export
plot_fitting_PCAWG_ALL_AT_ONCE <- function(filename,
                                           ABC,
                                           fit_ABC_routine) {
