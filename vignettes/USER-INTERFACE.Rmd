...
For a given cell, we have to know how many strands there are of each chromosome. For every strand, we have to provide the alleles at each bin.

Assume we have 20 cells at `T_0`. We can link the cells to clones sharing the same driver and CN profile.
...
`CN_arm` is a matrix of the baseline allele specification for each arm of each strand.
...
`CN_focal` overrides `CN_arm` if there are CN events targetting
...
The second clone contains 5 cells, assume that it has undergone the driver and CN events in order:

1. Gain TP53 mutation on strand B
2. Terminal CN-LOH on chromosome 17 from bin 1 to bin 20. Now the


replacing allele A with allele B (giving the TP52 mutation a second-hit).
3. Focal deletion on chromosome 3 from bin 50-70 on strand B
4. Whole-genome duplication
5. Gain BRAF mutation on strand B
6. Addition of chromosome-arm 10p

Then `CN_arm` should be `AABB` for every chromosome 1-X, except `AABBB` on chromosome-arm 10p.
`CN_focal` should account for the `BBBB` configuration on chromosome 17 on bin 1-20 and the `AA` configuration on chromosome 3 on bins 50-70.
There are driver mutations of TP53 and BRAF at 5 locations: TP53 on all strands of chromosome 17 and BRAF on strand 3 of chromosome 7 (which has allele B).

```{r}
cell_count        <- 5
CN_arm            <- c(rep('AABB',46),rep('',2))
CN_arm[21]        <- 'AABBB'
CN_focal          <- list(list(17,1,20,'BBBB'))
```
