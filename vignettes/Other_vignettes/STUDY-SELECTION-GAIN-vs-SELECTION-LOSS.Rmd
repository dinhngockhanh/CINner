...
model_prefix <- "GAIN-vs-LOSS"
folder_workplace <- "GAIN-vs-LOSS"
```

We set up parameter sweeps of the scale of selection of loss and gain chromosome arms.
For every parameter combination, we create 1000 simulations.
We set possible values for the selection scale of loss/gain chromosome arms to be from 1, 2,..., 9, 10.

```{r}
# n_simulations_per_batch <- 1000
n_simulations_per_batch <- 5

var1_name <- "scale_selection_loss"
# var1_vals <- seq(1, 10)
var1_vals <- seq(1, 2)
var1_labs <- paste0("x", var1_vals)

var2_name <- "scale_selection_gain"
# var2_vals <- seq(1, 10)
var2_vals <- seq(1, 2)
...
Here we can see how the statistics change by keeping the selection scale of gain arms constant for all values of selection scale for loss arms.
...
statistics_multivar_vector(
    ...
    var_labs = var1_labs, name_lab = "Scale of selection rates for TSG arms",
    plot_WGD = FALSE,
    n_simulations = n_simulations_per_batch,
    plotname = paste0(model_prefix, "_scale_selection_loss_varies_and_scale_selection_gain=", var2_fixed),
    example_simulation = FALSE,
    compute_parallel = FALSE
)
```

And we can see how the statistics change by keeping the selection scale of loss arms constant for all values of selection scale for gain arms.

```{r}
var1_fixed <- var1_labs[1]
statistics_multivar_vector(
    model_prefix = model_prefix, folder_workplace = folder_workplace,
    var1_name = var1_name, var1_labs = rep(var1_fixed, length(var2_labs)),
    var2_name = var2_name, var2_labs = var2_labs,
    var_labs = var1_labs, name_lab = "Scale of selection rates for ONCOGENE arms",
    plot_WGD = FALSE,
    n_simulations = n_simulations_per_batch,
    plotname = paste0(model_prefix, "_scale_selection_gain_varies_and_scale_selection_loss=", var1_fixed),
    example_simulation = FALSE,
    compute_parallel = FALSE
)
```
