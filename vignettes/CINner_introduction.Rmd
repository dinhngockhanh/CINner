---
title: "An introduction to CINner"
...
CINner requires many input parameters about the assumed characteristics of cancer growth. 
One easy method to load the parameters to `CINner` is by creating `model_variables`, 
which is a list containing relevant variables in a specific order, 
then using function `SAVE_model_variables` to save them as csv files that can be read in the next sections when the simulator is called. 
Here we detail how to create the parameter list.

## General variables for every cell

We first specify the cell lifespan (always in days). 
`cell_lifespan` is the mean of the exponentially distributed lifetime of every cell during cancer evolution, 
regardless of their driver mutation or CN profiles.
...
The unit should be the same for `T_0` and `T_end`.
...
`Table_sample` dictates the time points that each DNA sample is taken (same unit as the beginning and end time points), 
how many cells are in each sample, and the unique ID for each sample.
For example, the following line specifies that the sample in each `CINner` simulation is called "SA01" and consists of 300 cells, taken at 80 years.
...
Each CINner result then simulates the cell population at `T_0`, `T_0 + T_tau_step`, `T_0 + 2 * T_tau_step`, and so on, 
until `T_end`.
If not specified in `BUILD_general_variables`, `T_tau_step` is automatically set to be half of `cell_lifespan`. 
...
To speed up the simulations and in accordance with the current state of single-cell DNA sequencing, 
the genome is divided into bins of equal lengths (measured in number of base pairs).
The CN profile of each cell is represented as a vector, consisting of the CN of each bin.
If not specified in `BUILD_general_variables`, `CN_bin_length` is automatically set to 500,000 base pairs.
...
The local events target a randomly chosen region in a uniformly chosen chromosome arm in one of the daughter cells. 
All of these events require additional parameters for the region length distributions, to be discussed next. 
There are four local CNA mechanisms:
- Focal amplification and deletion duplicate or delete the region, respectively.
- Interstitial and terminal CNLOH replace all alleles within the region across chromosome strands with an allele chosen at random. 
