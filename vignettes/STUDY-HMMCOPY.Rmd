...
gc                                  <- read.csv(file = system.file("extdata", "gc_map_500kb-AU.csv", package = "CancerSimulator"))
gc_slope                            <- 1.2
gc_int                              <- 0
sigma1                              <- 0.1
num_reads                           <- 2e6
...
for (row in 1:length(vec.gc_slope)){
    for (col in 1:length(vec.sigma1)){
        ind         <- ind+1
        file.copy(paste(model_name, "_simulation_original.rda", sep = ""), paste(model_name, "_simulation_", ind,".rda", sep = ""))
