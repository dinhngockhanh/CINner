...
SIMULATOR_FULL_PHASE_1_CN_chrom_arm_missegregation <- function(genotype_to_react,
    ...
    if ((i_gain == 1) & (!is.null(genotype_daughter_2))) {
        ...
        chrom_ploidy <- ploidy_chrom_1[chrom]
        ploidy_allele_1[[chrom]][[chrom_ploidy]] <- matrix(0, nrow(ploidy_allele_1[[chrom]][[strand]]), ncol(ploidy_allele_1[[chrom]][[strand]]))
        ploidy_allele_1[[chrom]][[chrom_ploidy]][, block_start:block_end] <- ploidy_allele_1[[chrom]][[strand]][, block_start:block_end]
    } else if ((i_gain == 2) & (!is.null(genotype_daughter_2))) {
    ...
    if ((i_gain == 1) & (!is.null(genotype_daughter_2))) {
        ...
        chrom_ploidy <- ploidy_chrom_1[chrom]
        ploidy_block_1[[chrom]][[chrom_ploidy]] <- rep(0, length(ploidy_block_1[[chrom]][[strand]]))
        ploidy_block_1[[chrom]][[chrom_ploidy]][block_start:block_end] <- ploidy_block_1[[chrom]][[strand]][block_start:block_end]
    } else if ((i_gain == 2) & (!is.null(genotype_daughter_2))) {
        chrom_ploidy <- ploidy_chrom_2[chrom]
