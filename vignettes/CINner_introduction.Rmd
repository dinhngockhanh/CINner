...
We first load `CINner` and library `readxl` for reading Excel files:
...
The following code specifies that the sample in each `CINner` simulation is labeled "SA01" and consists of 300 cells, taken at 80 years.
...
table_gene_selection_rates <- data.frame(
    ...
    s_rate = runif(nrow(driver_gene_list), 1 / 1.2, 1.2),
    Chromosome = driver_gene_list$Chromosome,
    Bin = round(driver_gene_list$Start / CN_bin_length)
)
```
...
The final step of model variable preparation is using the function `SAVE_model_variables` to save the variables defined thus far in csv files. 
The variable `model_name` is incorporated into the csv files' names, and the simulator will look up files starting with this string to set up the simulations. 
Therefore, if different models are simulated in the same folder, they should be saved with different `model_name` so as not to overwrite older models.
