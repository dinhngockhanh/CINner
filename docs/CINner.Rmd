...
# rmarkdown::render("vignettes/CINner.Rmd")
```
...
Next, we specify the starting time point of the simulation using `T_0`.  
The simulation end time is automatically inferred from the sampling table (`Table_sample$Age_sample`). 
The unit can be
...
The unit for `T_0` should be consistent with the values in `Table_sample$Age_sample`.

```{r}
T_0 <- list(0, "year")
```

Table_sample specifies the time point at which the DNA sample is collected 
(in the same time unit as `T_0`).

The simulation end time is automatically inferred from the `Age_sample` column.

The table also defines:
- the number of cells in the sample, and
- the sample ID.

The following example specifies that the sample in each CINner simulation 
is labeled "SA01", consists of 300 cells, and is collected at 80 years.
...
until it reaches the final sampling time.
...
Here, we expect the cell population to follow a logistic expansion, growing from 20 cells at `T_0` to 10000 cells at the sampling time defined in `Table_sample$Age_sample`.
...
table_population_dynamics <- cbind(
    ...
    vec_cell_count = 10000 / (1 + exp(-0.3 * ((T_0[[1]]:Table_sample$Age_sample) - 20)))
)
```
...
`CINner_simulations[[1]]$neutral_variations` contains information from CINner's fourth step (neutral variations).
