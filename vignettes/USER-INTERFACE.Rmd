...
```{r}
Table_sample                        <- data.frame(Sample_ID='SA01',Cell_count=100,Age_sample=80)


# Table_sample                        <- data.frame(Sample_ID=c('SA01','SA02','SA03'),
#                                                 Cell_count=c(100,100,100),
#                                                 Age_sample=c(40,60,80))
```
...
MODEL_VARIABLES   <- BUILD_initial_population(MODEL_VARIABLES=MODEL_VARIABLES,
                    cell_count=cell_count,CN_matrix=CN_matrix,drivers=drivers)
```

We add another clone which is the same, except TP53 is mutated.
...
N_clones_min      <- 3
N_clones_max      <- 6
simulation        <- SIMULATOR_FULL_PROGRAM_one_simulation(model=MODEL_NAME,
                    stage_final=stage_final,N_clones_min=N_clones_min,N_clones_max=N_clones_max)
```

We first confirm that the population size in the simulation follows the input dynamics:

```{r,fig.height=4,fig.width=10}
PLOT_total_population_vs_input(simulation,MODEL_NAME,vec_time_plot=seq(0,80,by=1),unit='year')
```

We next plot the evolution of clones found in the sample.

```{r,fig.height=4,fig.width=10}
PLOT_clonal_evolution(simulation,vec_time_plot=seq(0,80,by=1),unit='year')
```

We can also look at the total CN profiles of the sampled cells.

```{r,fig.height=7,fig.width=10}
PLOT_copy_number_heatmap(simulation,phylo='true',plotcol='total_copy')
```

As well as their minor CN profiles.

```{r,fig.height=7,fig.width=10}
PLOT_copy_number_heatmap(simulation,phylo='true',plotcol='min_copy')
```

Finally, we take a look at the events that give rise to the clones found in the sample.
