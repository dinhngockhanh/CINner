...
#'
#' @description
#' `BUILD_cn_normal_autosomes` returns a dataframe of information about each autosome from a normal genome (chromosomes 1 to 22, each with 2 alleles A and B), including bin sizes and associated strand/allele.
#' 
...
#'
#' @description
#' `BUILD_cn_normal_XX` returns a dataframe of information about each chromosome that composes a normal female genome (chromosomes 1 to 22 and X, each with 2 alleles A and B), including bin sizes and associated strand/allele.
#' 
#' @param TABLE_CHROMOSOME_CN_INFO A dataframe containing columns "Chromosome", "Bin_count", "Centromere_location". "Chromosome" refers to the chromosome number. "Bin_count" refers to the number of bins in each chromosome. "Centromere_location" refers to the approximate base pair position of the centromere of the chromosome.
#' 
#' @examples
...
#'
#' @description
#' `BUILD_cn_normal_XY` returns a dataframe of information about each chromosome that composes a normal male genome (chromosome 1 to 22, each with 2 alleles A and B; and chromosomes X and Y, each with 1 allele A), including bin sizes and associated strand/allele.
#' 
#' @param TABLE_CHROMOSOME_CN_INFO A dataframe containing columns "Chromosome", "Bin_count", "Centromere_location". "Chromosome" refers to the chromosome number. "Bin_count" refers to the number of bins in each chromosome. "Centromere_location" refers to the approximate base pair position of the centromere of the chromosome.
#' 
#' @examples
...
#'
#' @export
BUILD_cn_normal_XY <- function(TABLE_CHROMOSOME_CN_INFO) {
    #--------------------------------------------------------Build CN matrix
    vec_chrom <- c()
    vec_bin_start <- c()
    vec_bin_end <- c()
    ...
    CN_strand_2 <- data.frame(vec_chrom, 2, vec_bin_start, vec_bin_end, "B")
    colnames(CN_strand_2) <- columns
    CN_strand_2 <- CN_strand_2[which(!CN_strand_2$Chromosome %in% c('X', 'Y')), ]
