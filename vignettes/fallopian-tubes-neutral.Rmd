...
- `stage_final`=2 also outputs the sample phylogeny
...
We first inspect the dynamics of the total cell population. The information is stored in entry 1 of the output package.
...
Now we explore the CN structures found in the sample. This is stored in entry 2 of the output package.

```{r}
sample_genotype_profiles      <- package_output[[2]]
# sample_genotype_profiles      <- package_sample_phylogeny[[1]]

head(sample_genotype_profiles)
```

We can use the `umap_clustering` function to infer the phylogeny tree and clustering of the sampled cells based on their CN profiles. This may not be the same as the true phylogeny tree and clustering from the simulation.

```{r}
cl_inferred                   <- umap_clustering(package_sample_phylogeny)
```

The CN profiles can be plotted using function `plotHeatmap` in library signals.
We first plot the total CN profiles.

```{r}
plotHeatmap(package_sample_phylogeny,plotcol="state",clusters=cl_inferred$clustering,tree=cl_inferred$tree,reorderclusters=TRUE,plottree=FALSE)
```

Then the plot of the minor CN profiles.

```{r}
plotHeatmap(package_sample_phylogeny,plotcol="Min",clusters=cl_inferred$clustering,tree=cl_inferred$tree,reorderclusters=TRUE,plottree=FALSE)
