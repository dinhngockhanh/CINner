...
get_cn_profile <- function(package_clonal_evolution, clone_ID) {
    ...
    for (chrom in 1:N_chromosomes) {
        ...
        #   Find location information for this chromosome
        ...
        #   Find CN for this chromosome ...
        if (clone_ID == 0) {
            #   ... for artificial nodes (intended for merging unmerged nodes at time 0)
            vec_copy <- rep(NA, 1, chrom_block_count)
            vec_state <- rep(NA, 1, chrom_block_count)
            vec_Min <- rep(NA, 1, chrom_block_count)
            vec_Maj <- rep(NA, 1, chrom_block_count)
        } else {
            #   ... for nodes with defined clonal CN
