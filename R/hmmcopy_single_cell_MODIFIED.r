stack_params_MODIFIED <- function(data, paramname) {
...
format_parameter_table_MODIFIED <- function(samp.segmented, new.params) {
    # mus - state medians
    ...
    loglik <- stack_params_MODIFIED(t(samp.segmented$loglik), "loglik")
    loglik$state <- NaN

    nus <- stack_params_MODIFIED(new.params$nu, "nus")
    nus$iteration <- NaN

    df.params <- rbind(
        stack_params_MODIFIED(samp.segmented$mus, "mus"),
        stack_params_MODIFIED(samp.segmented$lambdas, "lambdas"),
        stack_params_MODIFIED(samp.segmented$pi, "pi"),
        loglik, nus
    )
...
error_exit_clean_MODIFIED <- function(samp.uncorrected, chromosomes, sample_id, out_reads, out_segs, out_params, out_metrics, multiplier, error) {
...
# run_hmmcopy_MODIFIED <- function(cell, corrected_reads_data, param, outdir, multipliers, verbose = FALSE) {
run_hmmcopy_MODIFIED <- function(cell, corrected_copy, param, multipliers, verbose = FALSE) {
    #-----------------------------------------------------------Modified
    # samp.corrected <- fread(corrected_reads_data)
    # samp.corrected <- data.table(
    #     start = samp.corrected$start, end = samp.corrected$end, chr = samp.corrected$chr,
    ...
    if (all(is.na(check.samp.corrected$cor_gc)) | all(is.na(check.samp.corrected$copy))) {
        for (VAL in VALS) {
            ...
            error_exit_clean_MODIFIED(check.samp.corrected, chromosomes, opt$sample_id, out_reads, out_segs, out_params, out_metrics, VAL, err)
        }
    ...
    for (VAL in VALS) {
        ...
        df.params <- format_parameter_table_MODIFIED(samp.segmented, new.params)
    ...
    auto_ploidy.params <- best.params[[pick]]
    output <- list()
    output$auto_ploidy.reads <- auto_ploidy.reads
    output$auto_ploidy.segs <- auto_ploidy.segs
    output$auto_ploidy.metrics <- auto_ploidy.metrics
    output$auto_ploidy.params <- auto_ploidy.params
    return(output)
    #-------------------------------------------------------------------
}

get_parameters_MODIFIED <- function(str, e, mu, lambda, nu, kappa, m, eta, gamma, S) {
    str <- as.numeric(str)
    e <- as.numeric(e)
